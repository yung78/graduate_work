...
Для входа в сервис, в соответствующих полях предлагается ввести почту и пароль. Логин не стал использовать в аутентификации т.к. на данный момент  практически везде фигурирует почта(При необходимости быстро исправить...).
...
- почта - в соответствии с Т.З. 

Дефолтнй аккаунт админа имеет простой пароль(лень запоминать абракадабру).
**На все дальнейшие действия, связанные с изменениями почты и пароля будут распространяться условия валидации из Т.З.**
...
Для регистрации достаточно заполнить обязательные поля: "пароль" и "электронная почта"; остальные носят информативный характер. Строковые зачения почты и пароля проходят общую валидацию согласно Т.З.
...
При нажатии кнопки "изменить" отобразится окно для изменения имени файла и добавлении/изменении комментария:  
![изменить](./readme_res/change_file.png)  

Нажимая кнопку:  
- "изменить" мы подтверждаем изменения и отправляем их на сервер
- "отмена" отменяем внесение изменений  

Название файла проходит простую валидацию - поле не должно быть пустым. Если попытаемся сохранить пустое имя - отобразится предупреждение:  
![имя файла не пустое](./readme_res/file_name_error.png)
...
При отображении файлов списком, помимо комментария к файлу(если он есть) и дополнительной информации о файле можно увидеть иконку копирования ссылки:  
...
Для удобства способ отоборажения файлов сохраняется в **localforage**.
...
При изменении почты или пароля значение проходит стандартную валидацию. При изменении пароля дополнительно необходимо ввести старый пароль:  
...
Поля почты и пароля проходят стандартную валидацию. Если поля не проходят валидацию, отобразится сообщение об ошибке(отображение 3 секунды):  
...
При нажатии кнопки "детали" происходит переход на страницу карточки данного аккаунта. 
...
Под списком файлов расположены кнопки управления файлами карточки аккаунта:  
![управление файлами](./readme_res/card_file_buttons.png) 

Функционал такой же, как у кнопок управления на странице пользователя.  

В случае возникновения ошибки валидации файла аватара или ошибки фокусировки, отобразится сообщение об ошибке:  
![ошибка аватара](./readme_res/card_avatar_error.png)  

![ошибка фокуса](./readme_res/card_focus_error.png)  

## Итог

Не стал добавлять в админку изменение пароля (при необходимости добавлю на доработке). Так же была идея реализации поиска и фильтрации аккаунтов по различным параметрам(id, email, fullSize), но по времени уже затянул с выполнением, озможно при доработке допилю (сейчас аккаунты идут по возрастанию id).
